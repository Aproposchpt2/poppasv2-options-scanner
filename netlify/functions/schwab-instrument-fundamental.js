...
// POPPA'S Option Scanner v3 — Schwab instrument fundamental-data diagnostic function.
// Purpose: one-off live verification of the undocumented Schwab "fundamental" instrument
// fields (e.g. corpactionDate) against known earnings dates, using server-side OAuth.
// Security posture: market-data only, mirrors schwab-price-history.js. This function never
// calls Schwab account, trading, position, balance, order, transaction, or ACCT_ACTIVITY
// endpoints. It never returns Schwab access tokens, refresh tokens, client secrets, account
// IDs, account hashes, balances, positions, orders, or trading data.

const DEFAULT_TOKEN_URL = "https://api.schwabapi.com/v1/oauth/token";
const DEFAULT_API_BASE_URL = "https://api.schwabapi.com";
const INSTRUMENTS_PATH = "/marketdata/v1/instruments";
...
function assertAllowedMarketDataPath(pathname) {
  ...
  if (!ALLOWED_MARKET_DATA_PATHS.has(normalizedPath)) {
    const error = new Error(`Blocked Schwab path: ${normalizedPath}. Only market-data instruments path is allowed.`);
    error.status = 403;
    throw error;
  }
...
function buildInstrumentsUrl(url) {
  requireEnv(["SCHWAB_API_BASE_URL"]);
  assertMarketDataOnlyConfig();
  assertAllowedMarketDataPath(INSTRUMENTS_PATH);
  ...
  const iUrl = new URL(`${apiBase}${INSTRUMENTS_PATH}`);
  iUrl.searchParams.set("symbol", symbol);
  iUrl.searchParams.set("projection", "fundamental");

  return iUrl;
}

async function fetchInstrumentFundamental(url) {
  const { accessToken, metadata } = await refreshAccessToken();
  const iUrl = buildInstrumentsUrl(url);
  assertAllowedMarketDataPath(iUrl.pathname);

  const response = await fetch(iUrl.toString(), {
    method: "GET",
    headers: {
  ...
  try {
    ...
    parsed = { raw: "[non-json instruments response redacted]" };
  }

  if (!response.ok) {
    const error = new Error("Schwab instruments market data request failed.");
    ...
    error.safeDetails = {
      ...
      schwabError: parsed.error || null,
      schwabErrorDescription: parsed.error_description || null,
      requestedPath: iUrl.pathname,
      requestedSymbol: iUrl.searchParams.get("symbol")
    };
    throw error;
  }

  return { data: parsed, tokenMetadata: metadata, requestedUrl: iUrl };
}
...
export default async function handler(req) {
  ...
  try {
    ...
    const result = await fetchInstrumentFundamental(url);

    const payload = {
      ok: true,
      endpoint: "schwab-instrument-fundamental",
      diagnosticOnly: true,
      marketDataOnly: true,
    ...
    return json({
      ok: false,
      endpoint: "schwab-instrument-fundamental",
      error: error.message || "Schwab instruments function error.",
